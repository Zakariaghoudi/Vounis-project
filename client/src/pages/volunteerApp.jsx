...
console.log("all opportunities are:",allOpportunities);
    ...
    const getEnrichedApplications = () => {
        ...
        const userApps = allApplications.filter((a) => a.id_volunteer === userId);

        return userApps.map(app => {
            const opportunity = allOpportunities.find(opp => opp._id === app.id_opportunity);
            return {
                ...app,
                // Enriched Fields for display
                opportunityTitle: opportunity?.title || 'Title Not Found',
                opportunityLocation: opportunity?.location || 'N/A',
                hostId: opportunity?.id_host || 'N/A',
                // FIX: Status now reliably defaults to 'pending' from the updated model
    ...
    return (
        <div className="applications-container">
            ...
            <div className="applications-list grid-view"> {/* Added grid-view class */}
                ...
                {visibleApps.map((a) => (
                    <div key={a._id} className={`application-card card-grid status-${a.status}`}>
                        <div className="card-info">
                            <h4 className="project-title">{a.opportunityTitle}</h4>
                            <p className="host-name">
                                Host ID: <span>{a.hostId}</span>
                            </p>
